...
const Donate = (props) => {
    ...
    const onDonateFund = async () => {
        ...
        if(amount.value.length){
            let response = await sendDonation(donateId, amount.value, localStorage.getItem('token'));
    ...
    return (
        ...
        <div style={styles.conatiner}>
            <div style={{display: 'flex', flexDirection: 'column', paddingTop: '1.7rem'}}>
                <div style={{fontSize: '1.2rem', color: '#777777', marginBottom: '1rem'}}>Enter Details for Donating</div>
                <div style={{display: 'flex', justifyContent: 'space-between'}}>
                    <TextField sx={styles.textField} label="Enter Amount*" variant="standard" value={amount.value} onChange={(e)=>{setAmountData(e.target.value, false, '')}} error={amount.error} helperText={amount.errorText}/>
                </div>
                <br/>
