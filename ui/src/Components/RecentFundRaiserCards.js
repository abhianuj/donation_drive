import { Card, CardMedia, CardContent, Typography, LinearProgress, Button, Dialog, DialogTitle,Snackbar, Alert, DialogContent, DialogContentText, DialogActions, Link } from "@mui/material";
...
const RecentFundRaiserCards = ({ fundraiserData }) => {
    ...
    const [shareFundraiserData, setShareFundraiserData] = useState(null);
    ...
    const handleCopyToClipboard = () => {
        ...
        cb.writeText(shareFundraiserData).then(() => openSnackbar("URL copied to clipboard successfuly!", "success", true));
    }
    ...
    const openSnackbar = (text, severity, open) => {
        setSnackbar({
            severity: severity,
            open: open,
            snackbarText: text
                ...
                <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        ...
                        <div style={{display: 'flex', justifyContent: 'space-between'}}>
                            <Button variant="outlined" disabled={fundraiserData.neededAmount===fundraiserData.totalDonation} disableElevation onClick={()=> {donate(fundraiserData.id)}}>Donate</Button>
                            <Button variant="outlined" onClick={()=> {share(fundraiserData.id)}} disableElevation startIcon={<ShareIcon style={{width: '1rem'}}/>}>Share</Button>
                        </div>
                    </Typography>
                </CardContent>
            </Card>
